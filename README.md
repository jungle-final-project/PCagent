...
결과는 `docs/reports/as-chat-profile-benchmark-YYYYMMDD.md`에 생성됩니다. 일반 사용자 요청은 `AS_CHAT_DEFAULT_PROFILE` 하나만 실행하며, benchmark 스크립트만 내부 header로 profile들을 순차 비교합니다. `.env`에 `GEMINI_API_KEY`가 있거나 `--include-gemini`를 붙이면 `AS_CHAT_GEMINI_FAST`, `AS_CHAT_GEMINI_BALANCED`도 같은 평가셋으로 비교합니다. `/support/ai-chat` 화면은 `POST /api/ai/as-chat/stream`을 우선 사용해 첫 진행 상태와 최종 응답 시간을 따로 검증합니다.
