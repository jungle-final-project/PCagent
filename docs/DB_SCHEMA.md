...
| Recommendation event type | `IMPRESSION`, `CLICK`, `DETAIL_VIEW`, `SAVE`, `CHANGE_ADOPTED`, `ADD_BUILD_TO_DRAFT`, `ORDER_INTENT`, `REJECT`, `CHANGE_REVERTED`, `AS_CONFIRMED_NEGATIVE` |
...
목적: 추천 노출, 클릭, 저장, 변경 적용, 거절, 관리자 확정 AS negative feedback을 학습용 이벤트로 저장한다.
...
| `created_by_admin_id` | `BIGINT` | yes | `users.id` | AS feedback을 확정한 관리자 |
| `event_type` | `VARCHAR(50)` | no | - | 추천 행동 또는 `AS_CONFIRMED_NEGATIVE` |
| `label_score` | `NUMERIC(6,2)` | no | - | 학습 label 값 |
| `source_surface` | `VARCHAR(80)` | no | - | `BUILD_CHAT`, `HOME_RECOMMENDED_PARTS`, `SELF_QUOTE`, `ADMIN_AS_FEEDBACK` 등 |
...
- positive label은 `SAVE`, `CHANGE_ADOPTED`, `ADD_BUILD_TO_DRAFT`, `ORDER_INTENT`, weak positive는 `CLICK`, `DETAIL_VIEW`, negative는 `REJECT`, `CHANGE_REVERTED`, AS negative는 `AS_CONFIRMED_NEGATIVE`다.
...
- `RECOMMENDATION_RERANKER_ENABLED=false`가 기본이므로 실제 추천 순서를 바꾸지 않는다.
- `RECOMMENDATION_RERANKER_SHADOW_ENABLED=true`일 때 scorer가 정상 응답하면 점수만 저장한다.
