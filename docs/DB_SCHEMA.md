...
| AS chat session status | `ACTIVE`, `ADMIN_REVIEWING`, `TICKET_CREATED`, `ARCHIVED` |
| AS chat message role | `USER`, `ADMIN`, `SYSTEM`, `ASSISTANT` |
...
| `as_ticket_id` | `BIGINT` | yes | `as_tickets.id` | 기준 AS 티켓. 관리자 고객 연락 흐름에서는 티켓 생성 전까지 비어 있을 수 있음 |
...
관리자 고객 연락 흐름과 PC Agent 오류 접수 흐름에서는 상담방이 티켓보다 먼저 생성될 수 있으므로 `as_ticket_id`는 nullable이다. 관리자 상담 목록 표시와 티켓 발행 전 draft 보존을 위해 `assigned_admin_id`, `support_request_type`, `last_message_preview`, `last_message_at`, `admin_unread_count`, `user_unread_count`, `ticket_draft`를 사용할 수 있다.

MVP 기준 결정값:

- 한 사용자와 한 AS 티켓에는 active chat session 1개만 유지한다. 티켓 생성 전 PC Agent draft 상담방은 `ticket_draft.logUploadId`로 중복 생성을 막는다.
...
관리자 상담 메시지는 `role=ADMIN`, 시스템 안내는 `role=SYSTEM`을 사용할 수 있다. 첨부 메타데이터는 `attachment_metadata`에 저장하며, 읽음 처리는 MVP에서 `read_at`과 session unread count를 기준으로 한다.
