...
- XGBoost reranker는 1차에서 실제 추천 순서를 바꾸지 않는다. `RECOMMENDATION_RERANKER_ENABLED=false`, `RECOMMENDATION_RERANKER_SHADOW_ENABLED=true`가 기본이며, 외부 scorer가 응답한 경우에만 `recommendation_shadow_scores`에 점수를 저장한다. Tool `FAIL` 또는 하드 조건 위반으로 제외된 후보는 shadow scoring 대상에도 들어가지 않는다.
- 홈 하단 `인기 부품 랭킹`은 `GET /api/recommendations/home-parts`가 제공하는 내부 자산 추천부품 4개를 사용한다. 이 API는 견적 생성기가 아니라 쇼핑 발견용 부품 랭킹이며, XGBoost scorer가 실패하면 benchmark, 이미지/offer, 가격 갱신 신선도, toolReady, FPS coverage 기반 deterministic fallback을 반환한다.
- 추천 학습 이벤트는 `POST /api/recommendation-events`로 수집한다. AS 접수는 자동 negative label이 아니며, 관리자가 `/api/admin/recommendation-feedback/as-tickets/{id}`로 확정한 경우에만 `AS_CONFIRMED_NEGATIVE` 이벤트가 생성된다. 이 API는 `as_tickets.status`, `cause_candidates`, `upgrade_candidates`를 수정하지 않는다.
