...
| `llm` | seed 기반 RAG/Tool trace를 저장한 뒤 provider별 LLM API로 summary 생성 | `OPENAI_API_KEY` 또는 `GEMINI_API_KEY` | 실제 LLM 생성 결과가 관리자 화면까지 표시되는 검증 |
...
AS_CHAT_GEMINI_FAST_MODEL=gemini-2.5-flash
AS_CHAT_GEMINI_FAST_RAG_TOP_K=2
AS_CHAT_GEMINI_FAST_MAX_OUTPUT_TOKENS=900
AS_CHAT_GEMINI_BALANCED_MODEL=gemini-2.5-pro
AS_CHAT_GEMINI_BALANCED_RAG_TOP_K=3
AS_CHAT_GEMINI_BALANCED_MAX_OUTPUT_TOKENS=1200
...
AS Chat은 기본 요청에서 `AS_CHAT_DEFAULT_PROFILE` 하나만 실행한다. OpenAI/Gemini profile 비교는 `tools/benchmark_as_chat_profiles.py`가 내부 검증용 header `X-BuildGraph-AI-Profile`을 사용해 순차 실행한다. 사용자 화면은 `POST /api/ai/as-chat/stream`을 우선 사용해 `STARTED -> RAG_READY -> TOOLS_READY -> LLM_RUNNING -> DONE` 진행 상태를 표시한다.
...
| `AS_CHAT_GEMINI_BALANCED` | Gemini | 품질 비교 후보 | `gemini-2.5-pro` | `none` | 3 | 1200 |
...
- `llm` runner는 RAG evidence와 Tool invocation을 저장한 뒤 provider별 structured output API로 생성한 summary를 `agent_sessions.summary`에 저장한다.
